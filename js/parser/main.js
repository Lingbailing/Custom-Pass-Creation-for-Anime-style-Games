...
async function loadFont(family, style) {
   ...
   * 这个函数用来给文本找对应的字体
   */
  const usedStyle = style || 'Regular';
  const fontInfo = fontList[family];
  if (!fontInfo) throw new Error(`- 字体 ${family} 不存在 -`);
  const fontPath = fontInfo[usedStyle] || fontInfo['Regular'];
  if (!fontPath) throw new Error(`- 字体 ${family} 样式 ${usedStyle} 丢失 -`);
  // 统一使用 family::style 作为注册名，避免前端不同位置使用不同 key 导致不一致
  const fontKey = `${family}::${usedStyle}`;
